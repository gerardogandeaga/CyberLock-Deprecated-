package com.gerardogandeaga.cyberlock.lists.recyclerview.items;
...
public class ExternalLibItem extends AbstractItem<ExternalLibItem, ExternalLibItem.ViewHolder> {
    ...
    protected class ViewHolder extends FastItemAdapter.ViewHolder<ExternalLibItem> {
        ...
        public void bindView(@NonNull ExternalLibItem item, @NonNull List<Object> payloads) {
            ViewSetter.setOrHideTextView(item.mTitle, Title);
            ViewSetter.setOrHideTextView(item.mAuthor, Author);
            ViewSetter.setOrHideTextView(item.mDescription, Description);
            ViewSetter.setOrHideTextView(item.mUrl, Url);
        }
